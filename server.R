...
server <- function(input, output){
  ...
  catData <- reactive({
    if (is.na(input$id)){
      userdata <- data.frame(Trial=NA, Results=NA)
      return(userdata)
    }else{
      # Plant the random number seed
      set.seed(input$id) 
      ...
      colnames(userdata) <- c('Trial', 'Results')
      return(userdata)
    }
  }) # end reactive
  ...
  numData <- reactive({
    # Plant the random number seed
    if (is.na(input$id)){
      userdata <- data.frame(Bottle=NA, Volume=NA, Notes=NA)
      return(userdata)
    }else{
      set.seed(input$id) 
      n <- 60
      ...
      A_one <- 1:n
      A_two <- rnorm(n, 16.9, 0.05)
      A_three <- A_one %% 2
      
      userdata <- data.frame(A_one, A_two, A_three)
      col_headings <- c('Bottle', 'Volume', 'Notes')
  ...
  output$catTable <- DT::renderDT({
    datatable(catData(), rownames = FALSE, options = list(
      ...
      formatStyle("Results", backgroundColor=styleEqual(c("Correct","Incorrect"),c('orange','gray')))
  })
